...
驗證：244 項自動測試通過，涵蓋拳頭分類信心下降時延續追蹤、短暫／反覆漏偵測後恢復、身分衝突防護、背景辨識不阻塞、影像／結果配對、換鏡頭丟棄舊結果、圈形軌跡、計分與歷史回放；另檢查 1024×768 與 1920×1080 排版。
...
**可調測試參數：** 零、一的 60 秒、8 分與 0.3 秒位於 `common/timed_training.py`。三角形同步 0.3 秒及四指姿勢門檻位於 `common/straight_hand.py`：PIP／DIP 至少 155°、按壓 MCP 不大於 135°、四指 MCP 角度差不大於 25°；這些是影像判定初值，可依實測調整。
