...
### [點這裡下載最新版懶人包（ZIP）](https://github.com/QuailUJ/bilateral-coordination-training/archive/refs/heads/main.zip)

此連結下載 GitHub `main` 的最新版本，包含遊戲、模型、素材與自動安裝檔。**請先登入有此私人儲存庫權限的 GitHub 帳號**；若連結顯示 404，請確認帳號權限，也可回到儲存庫首頁點 **Code → Download ZIP**。

1. 下載 ZIP 後，按右鍵選 **全部解壓縮**。
2. 打開解壓後的資料夾，雙擊 **`install.bat`**，自動安裝 Python、執行環境與必要套件；第一次需要網路。
3. 等出現 **Installation complete!**，接上攝影機，雙擊 **`start_game.bat`** 開始玩。

**Windows 10／11（64 位元）適用。** 不必手動安裝 `requirements.txt`，也不要在 ZIP 裡直接執行。更新時請先關閉遊戲，解壓到新資料夾；要保留成績，將舊資料夾的 `data/` 複製過去。

## 最新更新（2026-09-12）：遊戲三還原舊版，只增加四指同時判定
