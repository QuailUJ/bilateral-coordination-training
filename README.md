...
- **三角形左右手把動畫修正**：手把改為跟隨四指相對手掌的平均下彎幅度，尚未達計分門檻也會逐漸壓下；維持下彎時不會因舊的 0.15 秒動畫時間到而回正。旋轉固定在灰色底座頂端，左右往中間壓。姿勢不合格顯示灰色且不能得分，計分規則維持。新版回放保存 `paddle_angle` 並重現手把動作，舊紀錄仍可播放。

**這次先看手把：** 重新啟動遊戲三，慢慢下彎四指，左右綠色手把應跟著逐漸壓下，抬手時回正；不需要先接到三角形才看得到動畫。手把動作與是否命中／得分是分開判定的。合成測試與畫面檢查通過，真人效果仍需確認。
...
驗證：280 項自動測試通過，包含未達計分門檻時手把仍會移動、維持下彎不因動畫逾時回正、回放角度保存、兩款遊戲共用姿勢判定、三角形首次入鏡立即下壓、漏偵測後恢復、持續下壓不重複結算同顆目標、雙藍／單手／捲指計分，以及光劍角度、拳頭追蹤、背景辨識與歷史回放。合成測試通過，真人手勢效果仍需更新後確認。
