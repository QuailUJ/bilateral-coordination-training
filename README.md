...
- **三角形按壓較容易觸發**：只調整遊戲三，四指下彎門檻由約 45° 改成約 30°（共同掌向角度 ≤150°），PIP／DIP 容許值由 155° 改成 145°，四指角度差由 25° 改成 35°，容納影像估計誤差。仍需四指一起動，明顯捲指無效。準備後短暫無效或漏偵測可保留最多 0.3 秒，無效影格不觸發；按壓後四指須抬回 ≥160° 才重新準備，避免抖動與按住重複觸發。狀態提示及按壓事件保存於回放姿勢資料。雙藍同步 0.3 秒與計分維持，真人適用程度仍需測試。

**三角形操作：** 更新並重新啟動後，先伸直四指，看到「已準備，請往下按」再從手掌相接處一起下彎；看到「已按下，請抬回」代表已辨識到下壓。每次按完先抬回準備下一次，雙藍進入手把判定區時兩手一起按才得分。
...
驗證：274 項自動測試通過，包含三角形約 32° 下壓、短／長漏偵測、門檻抖動不重複觸發、光劍左右水平端點、正負角度切換、4:3／16:9 攝影機角度換算、三角形從關鍵點輸入到按壓得分、光劍在攝影機影格之間轉動、四指姿勢、拳頭追蹤、背景辨識、計分與歷史回放；另檢查光劍與三角形畫面。
...
**可調測試參數：** 零、一的 60 秒、8 分與 0.3 秒位於 `common/timed_training.py`。三角形雙手同步 0.3 秒位於 `common/straight_hand.py`；遊戲三姿勢參數位於 `games/game4_bilateral_press/config.py`：PIP／DIP 至少 145°、四指共同掌向角度（`mcp`）皆 ≤150° 算下壓，皆 ≥160° 重新準備，四指角度差 ≤35°，短暫追蹤寬限 0.3 秒。光劍沿用共用預設 155°／25°。這些是影像判定初值，可依实測調整。
