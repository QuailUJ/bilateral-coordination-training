...
驗證：269 項自動測試通過，包含光劍左右水平端點、正負角度切換、4:3／16:9 攝影機角度換算、三角形從關鍵點輸入到按壓得分、光劍在攝影機影格之間轉動、四指姿勢、拳頭追蹤、背景辨識、計分與歷史回放；另檢查光劍與三角形 1024×768、2560×1440 畫面。
...
**可調測試參數：** 零、一的 60 秒、8 分與 0.3 秒位於 `common/timed_training.py`。三角形同步 0.3 秒及四指姿勢門檻位於 `common/straight_hand.py`：PIP／DIP 至少 155°、四指相對共同掌向的角度（紀錄欄位 `mcp`）不大於 135°才算下壓，四指角度差不大於 25°；這些是影像判定初值，可依實測調整。
